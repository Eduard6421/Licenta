...
public class tempKinematic : MonoBehaviour
{
    ...
    void UpdateSteering()
    {
        ...
        else
        {
            ...
            rotation += steering.angularAcceleration;
    ...
    void Update()
    {
        ...
        CurrentTargetOrientation = Target.transform.rotation.eulerAngles.y;
        newUpdate();
    ...
    void newUpdate()
    {
        ...
        if (Target != null && steeringType == null)
        {
            if(this.gameObject.name == "Runner")
            {
                steeringType = BehaviourFactory.GetCharacterBehaviour(Utilities.Behaviour.LookAndRun, MaxSpeed, MaxRotation, new List<Vector3> { }, new List<GameObject> { });
            }
            else
            {
                Target = GameObject.Find("Runner");
                CurrentTargetPosition = Target.gameObject.transform.position;
                CurrentTargetOrientation = Target.gameObject.transform.eulerAngles.y;
                TargetVelocity = Target.GetComponent<tempKinematic>().GetVelocity();
                TargetRotation = Target.GetComponent<tempKinematic>().GetRotation();

                steeringType = BehaviourFactory.GetCharacterBehaviour(Utilities.Behaviour.Pursue, MaxSpeed, MaxRotation, new List<Vector3> { }, new List<GameObject> { });

            }

        }
        else if (steeringType != null)
        {
            this.transform.position += velocity * Time.deltaTime;
            this.transform.Rotate(Vector3.up * rotation);
            IsKinematicTarget();

            if(Target.GetComponent<tempKinematic>() != null)
            {
                TargetVelocity = Target.GetComponent<tempKinematic>().GetVelocity();
                TargetRotation = Target.GetComponent<tempKinematic>().GetRotation();
